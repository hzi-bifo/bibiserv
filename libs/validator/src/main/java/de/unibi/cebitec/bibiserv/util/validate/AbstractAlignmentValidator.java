...
//import de.unibi.techfak.bibiserv.biodom.toolbox.SequenceValidator;
 ...
 */
public abstract class AbstractAlignmentValidator extends AbstractSequenceValidator implements AlignmentValidator {
    ...
    protected ValidationResult standardSequenceValidationSteps(Object data) {
        ValidationResult vr = super.standardSequenceValidationSteps(data);
        if (vr.isValid() && isAlignment() && !getSequenceCollection().isAllEqualLength()) {
            vr = new ValidationResult(false, "Data isn't an alignment!");
        }
        return vr;
    }
    // ############################## private ##############################
