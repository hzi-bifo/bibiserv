...
/**
 * General Implementation of a Dialign Validator. Dialign is an alignment only
 * format. Validator suppresses all sequence related features. 
 * 
 * @author Jan Krueger - jkrueger(at)cebitec.uni-bielefeld.de
 */
public class Dialign_Validator extends AbstractAlignmentValidator {

    public Dialign_Validator() {
        super();
    ...
    public void setCardinality(CARDINALITY cardinality) {
        if (!cardinality.equals(CARDINALITY.multi)) {
            throw new UnsupportedCardinalityException("Dialign is an alignment only format. Therefore any other cardinality than 'multi' aren't supported!");
        }
