 ...
 */
public class MultiMap<X, Y> {

    HashMap<X, HashSet<Y>> actualMap;
     ...
     */
    public HashSet<Y> put(X key, Y value) {
        HashSet<Y> values = actualMap.get(key);
        if (values == null) {
            values = new HashSet<>();
        }
        values.add(value);
    ...
    public HashSet<Y> valueSet() {
        ...
        for (HashSet<Y> values : actualMap.values()) {
            copy.addAll(values);
        }
        return copy;
    }
