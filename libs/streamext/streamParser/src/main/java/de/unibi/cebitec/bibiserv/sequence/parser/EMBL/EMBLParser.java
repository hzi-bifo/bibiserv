...
/**
 * A Parser to validate and correct EMBL files.
 *
 * @author Thomas Gatter - tgatter@cebitec.uni-bielefeld.de
 */
public class EMBLParser extends AbstractSequenceParser {

    private EMBLParserState state;
    ...
    private static final String NUMBER_INDENT = " ";
     ...
     */
    public EMBLParser(BufferedReader input, BufferedWriter output,
    ...
    protected void lineEmpty() throws SequenceParserException {
        if (state != EMBLParserState.seekSequenceStart) {
    ...
    protected int lineBegin(String value) throws SequenceParserException {
        if (state != EMBLParserState.readSequence && value.length() < 2) {
        ...
        switch (state) {
            case seekSequenceStart:
                ...
                state = EMBLParserState.seekSequence;
            ...
            case seekSequence:
                ...
                if (value.startsWith("SQ")) {
                    state = EMBLParserState.readSequence;
    ...
    protected void lineEnd(int ident, String value) throws SequenceParserException {
        ...
        switch (ident) {
            ...
            case SEQUENCEHEADER:
                sequenceHeaderTmp += value;
                if (!sequenceHeaderTmp.trim().split("\\s")[0].startsWith("Sequence")) {
    ...
    private String writeSequence(String sequence, boolean finalize) throws SequenceParserException {
        while (sequence.length() > SEQUENCE_WIDTH) {
            ...
            writeSequenceNumber(SEQUENCE_WIDTH);
        ...
        if (finalize) {
            ...
            write(tmp.toString());
            writeSequenceNumber(sequence.length());
     ...
     */
    private void writeSequenceNumber(int len) throws SequenceParserException {

        write(NUMBER_INDENT);

        String num = Integer.toString(sequenceWritten * SEQUENCE_WIDTH + len);
