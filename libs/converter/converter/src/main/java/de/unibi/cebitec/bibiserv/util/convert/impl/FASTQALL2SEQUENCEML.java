...
import de.unibi.cebitec.bibiserv.util.fastqconvert.converter.SequenceMLConverter;
import java.io.File;

/**
 * Convert all fastq variants to sequenceml, leaving quality score as comment.
 * @author Thomas Gatter - tgatter(at)cebitec.uni-bielefeld.de
 */
public class FASTQALL2SEQUENCEML implements Converter{

    @Override
    public Object convert(Object fromdata) throws ConversionException {
        if (fromdata instanceof java.lang.String) {
            
            SequenceMLConverter converter = new SequenceMLConverter();
            try {
                return converter.convertToSeqenceML((String) fromdata);
            ...
            SequenceMLConverter converter = new SequenceMLConverter();
            try {
                return converter.convertToSeqenceML((File) fromdata);
            } catch (FastQConversionException ex) {
                throw new ConversionException(ex.getLocalizedMessage());
            }
        }
